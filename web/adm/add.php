	      	...
	      	if($_POST['senha'] == $_POST['resenha'])
	      	{
				...
				$sql_insert = "INSERT INTO store
								           (subtype, city, 
								           	...
								           	address, 
								           	cnpj, site, 
								           	idStoreType, vip, password) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			    ...
			    $stmt->bindValue(6, $_POST['telefone1']);
			    $stmt->bindValue(7, $_POST['telefone2']);
			    $stmt->bindValue(8, $_POST['telefone3']);
			    $stmt->bindValue(9, $_POST['endereco']);
			    $stmt->bindValue(10, $_POST['cnpj']);
			    $stmt->bindValue(11, $_POST['site']);
			    $stmt->bindValue(12, $rowSub[0]['Store_Type_id']);
			    $stmt->bindValue(13, $_POST['vip']);
			    $stmt->bindValue(14, md5($_POST['password']));
			    $stmt->execute();
							...
							<div class="panel-body">
								...
								<form method="post" enctype="multipart/form-data">
									<div class="form-group">
										...
										<div class="row">
											<div class="col-sm-6">
												<select class="form-control select2" name="estabelecimento[]" multiple="multiple" required>
													...
													<optgroup label="<?php echo utf8_encode($rowMenu[$i]['name']); ?>">
														<?php 
															...
															for ($j=0; $j < sizeof($rowSub); $j++) { 
																echo '<option value="' . $rowSub[$j]['id'] . '"> - ' . $rowSub[$j]['name'] . '</option>';
															}
														?>
													</optgroup>
													<?php } ?>
