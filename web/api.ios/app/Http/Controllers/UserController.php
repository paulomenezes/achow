...
class UserController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $iostoken = null;
        if (array_key_exists('iostoken', Input::json()->all())) {
            $iostoken = Input::json()->all()['iostoken'];
        }

        $users = \App\Models\User::where('email', $email)->where('password', $password)->get();
        if (sizeof($users) > 0) {
            $users[0]->gcm_regid = $gcmid;
            $users[0]->iostoken = $iostoken;
            $users[0]->save();
        }
    ...
    public function store(Request $request)
    {
        ...
        if (array_key_exists('iostoken', Input::json()->all())) {
            $user->iostoken = Input::json()->all()['iostoken'];
        } else {
            $user->iostoken = null;
        }
        $user->save();
    ...
    public function storeFriendRequests(Request $request)
    {
        ...
        if ($user2['iostoken']) {
            $result = $this->send_ios($user2['iostoken'], "Você tem uma solicitação de amizade.");
        }
    ...
    public function notification($id, $msg)
    {
        ...
        $result = $this->send_ios($user->iostoken, $msg);
